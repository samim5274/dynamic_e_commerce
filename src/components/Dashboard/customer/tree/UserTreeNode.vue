<template>
  <div class="flex flex-col items-center flex-nowrap">
    ...
    <div class="relative flex flex-col items-center group">
      <div
        ...
        :class="[
          ...
          user?.id
            ? 'border-[#A3D921] hover:shadow-lg hover:-translate-y-1'
            : 'border-dashed border-slate-300 dark:border-slate-700 opacity-60 bg-slate-50/50',
        ]"
      >
        <!-- Avatar -->
        <div
        ...
        >
          <i
            :class="[
              'fa-solid text-sm sm:text-lg',
              user?.id ? 'fa-user text-[#A3D921]' : 'fa-plus text-slate-400',
            ]"
          ></i>
        </div>

        <h4
          class="text-[10px] sm:text-[11px] font-bold truncate dark:text-white uppercase tracking-tight leading-tight"
        >
          {{ user?.name || "Available" }}
        </h4>
        ...
        <p
          v-if="user?.id"
          class="text-[8px] sm:text-[10px] text-[#3cabd3] font-mono mt-0.5"
        >
          {{ user.user_id }}
        </p>
        ...
        <div
          v-if="user?.id"
          class="grid grid-cols-2 gap-1 mt-2 pt-2 border-t border-gray-100 dark:border-slate-700"
        >
          <div class="text-left">
        ...
        <div v-if="user?.id" class="mt-3 flex justify-center">
          <div
            v-if="user?.is_active"
            class="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[9px] font-bold uppercase"
          >
            <span
          ...
          <div
            v-else
            class="flex items-center gap-1 px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-600 dark:text-rose-400 text-[9px] font-bold uppercase"
          >
    ...
    <div v-if="shouldShowChildren" class="flex flex-nowrap relative">
      <div
        class="absolute top-0 left-[25%] right-[25%] h-0.5 bg-[#A3D921]/40"
      ></div>
      ...
      <div class="flex gap-4 sm:gap-12 flex-nowrap">
        ...
        <div class="relative pt-8">
          ...
          <UserTreeNode
            :user="
              user?.left_child_recursive || user?.leftChildRecursive || null
            "
            :depth="depth + 1"
            :parentUser="user"
            position="left"
        ...
        <div class="relative pt-8">
          ...
          <UserTreeNode
            :user="
              user?.right_child_recursive || user?.rightChildRecursive || null
            "
            :depth="depth + 1"
            :parentUser="user"
            position="right"
...
function handleClick() {
  if (props.user?.id) {
    ...
    router.push(
      `/create-user/${props.parentUser.user_id}?position=${props.position}`,
    );
  }
